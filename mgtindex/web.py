...
<header>
  ...
  <div id="settings">
    ...
    <p>A link opens the book at the right page (via <code>#page=</code>), at whatever zoom your
       PDF viewer is set to. It does not scroll to the exact paragraph — browser PDF viewers
       handle that inconsistently, so the index aims the page and lets you read from the top.</p>
...
// Open the book at the target page. We deliberately send ONLY #page=N and no view/zoom.
// The obvious richer form -- #page=N&view=FitH,<top> to scroll the paragraph into view --
// backfires in Chrome's PDF viewer: FitH means "fit the page WIDTH", which over-magnifies on
// a wide window, and Chrome reads the <top> coordinate upside down (PDF space is measured
// from the bottom), landing you on the page-number footer instead of the paragraph. Bare
// #page=N lands at the top of the right page at the reader's own zoom -- predictable, and it
// is what a back-of-book index promises anyway: the page. (`top` is still precomputed and
// carried in the data, so a Chrome-verified precise-scroll mode can be added later.)
const url = r => pathFor(r[0]) + "#page=" + r[2];
